...
```{r}
describe(HCS)
### Get rid of time points  6,7,8 because there is sparse data for them
...
### Total change in program for entire time (6 time points)
model_int_summary$coefficients[2,1]*6
...
bayes_bmi_null = brm(bmi ~ + (1 | universal_id), data = CIL_CKY_vitals_complete, cores = 3, chains = 3, thin = 10)

icc(bayes_bmi_null)

waic_bmi_null = WAIC(bayes_bmi_null)

## Intercept and slopes 
bayes_bmi_inter_slopes = brm(bmi ~ timepoints_vitals + bp_systolic + gender + ageatmodelenrollment + (timepoints_vitals | universal_id), data = CIL_CKY_vitals_complete, cores = 3, chains = 3, thin = 10)

waic_bmi_inter_slopes = WAIC(bayes_bmi_inter_slopes)

compare_ic(waic_bmi_null, waic_bmi_inter_slopes)

LOO(bayes_bmi_null, bayes_bmi_inter_slopes)
...
#####################
```{r}
summary(bayes_bmi_inter_slopes)

## checking the range
test = pp_check(bayes_bmi_inter_slopes)
range(test$data$value)

launch_shinystan(bayes_bmi_inter_slopes)

pred_y = posterior_predict(bayes_bmi_inter_slopes)

CIL_CKY_vitals_complete$pred_y = dim(pred_y)
summary(bayes_bmi_inter_slopes)

plot_model(bayes_bmi_inter_slopes, type = "pred", terms = c("timepoints_vitals", "ageatmodelenrollment"))
```
